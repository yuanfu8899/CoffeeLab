...
*   **📝 沖煮手法編輯**：可自訂您的專屬沖煮流程 (注水/悶蒸/等待時間)。
    *   **🎯 比例式水量目標**：支援基於粉量倍率的自動水量計算 (例如：2D, 6D, 12D, 15D)，當調整粉量時，各步驟目標水量會自動更新。
...
Coffee Lab 支援兩種方式設定步驟的目標水量：

### 1. 絕對值模式 (傳統方式)
直接設定累計水量的絕對克數，例如：40g, 120g, 240g, 300g

### 2. 比例式模式 (新功能)
設定相對於粉量的倍率，目標水量會自動計算。例如：
- 設定倍率：2, 6, 12, 15
- 粉量 D = 20g 時，目標水量為：40g, 120g, 240g, 300g
- 粉量 D = 15g 時，目標水量自動變為：30g, 90g, 180g, 225g

**公式**：目標水量 = 粉量 × 倍率

**使用方式**：
1. 編輯沖煮手法時，在步驟設定中可以看到「或倍率」欄位
2. 輸入倍率數值 (例如 2, 6, 12, 15)
3. 在計時器頁面調整粉量時，各步驟的目標水量會自動重新計算

**範例手法**：V60 四段注水 (比例式)
- 悶蒸：倍率 2 (粉量 × 2)
- 第一段：倍率 6 (粉量 × 6)
- 第二段：倍率 12 (粉量 × 12)
- 第三段：倍率 15 (粉量 × 15，對應 1:15 粉水比)
